...
void hashmap::put(int key, int value) {
  ...
  int bucketInd = hashFunction(key) % nBuckets;

  //checks if key already exists in map
  key_val_pair* curr = buckets[bucketInd];
  ...
  //if key does not exists, create new node and add it to linked list
...
int hashmap::get(int key) const {
  ...
  int bucketInd = hashFunction(key) % nBuckets;
  //search for key in the linked list of the bucket
  key_val_pair* curr = buckets[bucketInd];
  while(curr != nullptr){
      if(curr -> key == key){
          return curr -> value;
